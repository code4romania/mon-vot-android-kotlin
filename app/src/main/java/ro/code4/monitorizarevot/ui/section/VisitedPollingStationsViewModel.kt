...
class VisitedPollingStationsViewModel(
    ...
    private val _visitedStations = MutableLiveData<Result<List<CountyMunicipalityAndPollingStation>>>()
    val visitedStations: LiveData<Result<List<CountyMunicipalityAndPollingStation>>> = _visitedStations
    val hasUnsentData: LiveData<Result<Boolean>> = _hasUnsentData

    init {
        val subscription = repository.getVisitedStations()
            .subscribeOn(Schedulers.io())
            .map { dbData -> Result.Success(dbData) as Result<List<CountyMunicipalityAndPollingStation>> }
